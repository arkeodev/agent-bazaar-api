...
def login_user(username: str, password: str) -> Optional[dict]:
    ...
    if response.status_code == 201:
        ...
        logger.error(f"Login failed for user {username}. Status code: {response.status_code}, Response: {response.text}")
    return None

def register_user(username: str, email: str, full_name: str, password: str, password_confirm: str) -> bool:
    logger.info(f"Attempting to register user: {username} on {API_BASE_URL}/api/v1/user")
    ...
    try:
        response = requests.post(
            ...
            json={"username": username, "email": email, "name": full_name, "password": password}
        )
        ...
        if response.status_code == 201:
            ...
            logger.error(f"Registration failed for user {username}. Status code: {response.status_code}, Response: {response.text}")
            return False
...
def logout_user():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.error(f"Logout failed. Status code: {response.status_code}, Response: {response.text}")
            return False
    except requests.RequestException as e:
